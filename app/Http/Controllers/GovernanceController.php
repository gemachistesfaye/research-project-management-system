...
class GovernanceController extends Controller
{
    ...
    public function exportAnalyticsCsv()
    {
        ...
        $callback = function () use ($projects) {
            ...
            // Standard Clean Tabular Columns for direct Excel sorting / Pivot tables
            fputcsv($handle, [
                'Project Code',
                'Project Title',
            ...
            foreach ($projects as $p) {
                $reqBudget = (float) $p->requested_budget;
                $appBudget = (float) ($p->approved_budget ?? 0);
                ...
                fputcsv($handle, [
                    ...
                    $p->pi->name ?? 'N/A',
                    number_format($reqBudget, 2, '.', ''),
                    number_format($appBudget, 2, '.', ''),
                    $tier,
                    str_replace('_', ' ', $p->status),
                    $p->created_at ? $p->created_at->format('Y-m-d') : 'N/A'
                ]);
            }
